    ...
    public PontosButton(String text, MatchPlayerStats stats, OperatorButton operatorButton,
        ...
        this.addActionListener(e -> {
            if (operatorButton.isSomando()) {
                stats.addPonto();
                switch (teamID) {
                    case TIME_A -> matchManagerFrame.adicionarPontoTimeA().ifPresent(matchManagerFrame.getConfirmarAvancarSet());
                    case TIME_B -> matchManagerFrame.adicionarPontoTimeB().ifPresent(matchManagerFrame.getConfirmarAvancarSet());
                }
